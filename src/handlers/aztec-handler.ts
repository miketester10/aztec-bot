...
`
  ...
  private async getAllValidators(): Promise<AllValidatorsResponse> {
    ...
    try {
      ...
      const mainResponse = await axios.get<AllValidatorsResponse>(`${API.VALIDATORS_STATS}?page=1&limit=200`, { httpsAgent: firstProxy, headers: firstHeaders });
      ...
      for (let page = 1; page <= allValidatorsResponse.totalPages; page++) {
        ...
        requests.push(
          limit(async (): Promise<Validator[]> => {
            try {
              const response = await axios.get<AllValidatorsResponse>(`${API.VALIDATORS_STATS}?page=${page}&limit=200`, { httpsAgent: proxyAgent, headers: browserHeaders });
